"""tiberio-setup — one-shot setup orchestrator (KONZEPT §5/§6).
...
# tiberio/cli/setup.py -> tiberio/cli -> tiberio -> project root
...
SECRET_KEYS = ("TIBERIO_JWT_SECRET", "TIBERIO_SHARED_SECRET")
...
def _deploy_infra(
    ...
    *,
    ...
    env = os.environ.copy()
    if env_file.exists():
        shared = parse_env(env_file.read_text()).get("TIBERIO_SHARED_SECRET", "")
        if shared:
...
def _link(
    ...
    *,
    ...
    for path in (manifest_file, linking_file):
        if not path.exists():
            _fail(f"{path} not found — run 'tiberio-setup render' first")
...
def _print_manual_steps() -> None:
    ...
    typer.echo(
        ...
        "TIBERIO_OAUTH_ALLOWED_REDIRECT_URIS (comma-separated) and restart the server."
    )
...
def infra(
    ...
    env_file: Annotated[
        Path,
        typer.Option("--env-file", help=".env to read TIBERIO_SHARED_SECRET from."),
    ] = DEFAULT_ENV_FILE,
...
def run_all(
    ...
    if username:
        typer.echo("== User ==")
        run_command(["uv", "run", "tiberio-users", "add", username])

    if base_url:
        typer.echo("== Beacon ==")
        run_command(["uv", "run", "tiberio-beacon", "publish", "--base-url", base_url])
