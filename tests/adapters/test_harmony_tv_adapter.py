"""Tests for HarmonyTvAdapter using an injected fake hub."""
...
def _adapter(hub: FakeHub) -> HarmonyTvAdapter:
    return HarmonyTvAdapter("192.168.1.50", hub=hub)  # type: ignore[arg-type]
...
class TestLifecycle:
    async def test_start_connects_hub(self) -> None:
        hub = FakeHub()
        await _adapter(hub).start()
        assert hub.connect_count == 1

    async def test_stop_closes_hub(self) -> None:
        hub = FakeHub()
